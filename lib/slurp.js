 ...
 * back with a value, not a constructor.
 *
 * @param {string} name
...
Slurp.prototype.resolve = function(name, callback) {
  ...
  // really getting a constructor, not a value
  if (value = this.registry[name]) {
    ...
    this.once(name, function(constructor) {
      self.spinning++;
      constructor(function(err, value) {
        if (err) {
          return self.emit('error', err);
        }
        self.spinning--;
...
Slurp.prototype.service = function(name, dependencies, callback, context) {
  ...
  this.exec(dependencies, function() {
    var running = false, args = slice.call(arguments);
    ...
    self.constructor(name, function(done) {
      if (!running) {
        running = true;
        callback.apply(context, args);
      }
      self.once(name, function(constructor) {
        constructor(done);
      });
    });
  });
...
Slurp.prototype.factory = function(name, dependencies, callback, context) {
  ...
  this.exec(dependencies, function() {
    ...
    args.push(function(err, constructor) {
      if (err) {
        return self.emit('error', err, name);
      }
      self.spinning--;
...
Slurp.prototype.constructor = function(name, constructor, context) {
  ...
  process.nextTick(this.emit.bind(this, name, constructor));
};
...
Slurp.prototype.value = function(name, value) {
  this.constructor(name, function(callback) {
    callback(null, value);
  });
};
